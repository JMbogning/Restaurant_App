...
const ActiveOrderScreen = ({ history }) => {
    ...
    const renderFreeTables = () =>
        filterTablesByState(false).map((table) => (
            <Link
                to={`/orderCreate/${table.id}/table`}
                key={table.id}
                className="btn btn-block btn-success btn-lg mb-1"
            >
                <p className="text-center my-0">
                    <i className="float-left my-1 bi bi-table"></i>
                    {table?.name}
                </p>
            </Link>
        ));
                ...
                <div className="container-fluid">
                    <div className="row">
                        ...
                        <div className="col-12 col-md-3 col-lg-3">
                            <div className="card">
                                <div className="card-header">Tables Libres</div>
                                <div className="card-body">
                                    <LoaderHandler
                                        loading={loading}
                                        error={error}
